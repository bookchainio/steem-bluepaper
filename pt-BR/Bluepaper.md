...
Essas chaves privadas são as de Publicação, Ativa e Proprietário. A chave de publicação permite que contas publiquem, comentem, editem, votem, resteemem[^15], e sigua/silencie outras contas. A chave ativa é para tarefas mais sensíveis como a transferência de fundos, transações de aumento/redução de poder, conversão de Dólares Steem, votação para testemunhas, colocar ordens no mercado, e e redefinir a chave de publicação. A chave de proprietário serve apenas para uso quando necessário. It is the most powerful key because it can change any key of an account, including the owner key, and to prove ownership during an Account Recovery. Ideally it is meant to be stored offline, and only used when the account’s keys need to be changed or to recover a compromised account.
