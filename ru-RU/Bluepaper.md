...
В отличие от криптовалют, выработанных только PoW, токены Steem генерируются по фиксированной ставке одного блока каждые 3 секунды. These tokens get distributed to various actors in the system based on the defined rules of the blockchain. These actors, such as content creators, witnesses, and curators, compete in specialized ways for the tokens. Unlike the traditional PoW means of distribution, where miners are competing over raw computing power, the actors in the Steem network are incentivized to compete in ways that add value to the network.
