...
Steem stosuje pierwszą w swoim rodzaju hierarchiczny sytem prywatnych kluczy aby ułatwić transakcje z małymi i duzymi zabezpieczeniami. Transakcje z małymi zabezpieczeniami są raczej społeczne jak postowanie i komentowanie. Transakcje z dużymi zabezpieczeniami są raczej przelewami i zmianami kluczy. Pozwala to użytkownikom wprowadzać rózne poziomy bezpieczeństwa dla ich kluczy, w zależności od dostępu na jaki klucze pozwalają.

Te prywatne klucze to Postowanie, Aktywność oraz Właściciel. Klucz postowania pozwala kontom postować, komentować, edytować, głosować, resteem'ować[^15] i podążać/wyciszać inne konta. Klucz aktywności przeznaczony jest do bardziej wrażliwych zadań jak np. przelew funduszy, transakcje power up/down, kowersja Steem Dollars, głosowanie na świadków, składanie ofert na giełdzie i resetowanie klucza postowania. Klucz właściciela jest przeznaczony do użytku tylko jeżeli jest to konieczne. It is the most powerful key because it can change any key of an account, including the owner key, and to prove ownership during an Account Recovery. Ideally it is meant to be stored offline, and only used when the account’s keys need to be changed or to recover a compromised account.
